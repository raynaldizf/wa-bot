...
const configuration = new Configuration({
    apiKey: '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',
});
...
function man() {
    client.on('message_create', async message => {
        if (message.body.includes('!ask')) {
        ...
        else if (message.body.includes('!img')) {
            let text = message.body.split('!img')[1];
            var qst = `Q: ${text}\nA:`;
            ...
            await client.sendMessage(message.from, media, { caption: "Ni gambarnya bos" })
        }
        else if (message.body.includes('!everyone')) {
            let text = message.body.split('!everyone')[1];
            var qst = `Q: ${text}\nA:`;
            const response = await openai.createCompletion({
                model: "text-davinci-003",
                prompt: qst,
